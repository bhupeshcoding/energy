...
//     // @ts-ignore - autoTable is added to jsPDF prototype by jspdf-autotable
//     // @ts-ignore - TypeScript doesn't know about the autoTable extension
//     doc.autoTable({
//       startY    : 42,
//       head      : [['Category', 'Your Response']],
//       body      : fields.map(f => [f.label, data[f.key] || 'Not specified']),
...
//         }
//         await generatePdf(stored?.survey ?? {});
...
export function Survey() {
  ...
  // Memoize tips to prevent unnecessary re-renders
  const tips = useMemo(() => [
    'Seal gaps around doors & windows with weather-stripping.',
    'Upgrade to LED bulbs – they use 75% less energy.',
    'Replace or clean HVAC filters every 3 months.',
  ...
  // Load data and generate PDF on mount
  useEffect(() => {
    const run = async () => {
      if (!scanId) { 
        toast.error('No scan ID provided');
